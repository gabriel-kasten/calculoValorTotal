...
function calcularPrecoOutsourcing(escolha) {
    let handle = outsourcingOpcoes.planos.find(p => p.nome === escolha);
    return handle.valor;
}

function calcularPrecoMaquinas(quantidadeMaquinas = 0, quantidadeServidoresFisicos = 0, quantidadeServidoresVirtuais = 0) {
    return (quantidadeMaquinas * 89.90) + ([quantidadeServidoresFisicos + quantidadeServidoresVirtuais] * 280);
}
...
const quantidadeMaquinas = 1;
const quantidadeServidoresFisicos = 1;
const quantidadeServidoresVirtuais = 1;

const preco = calcularPrecoHora(horasEscolhidas, nivelTecnologia) + calcularPrecoOutsourcing(outsourcing) + calcularPrecoMaquinas(quantidadeMaquinas, quantidadeServidoresFisicos, quantidadeServidoresVirtuais);
console.log(preco);